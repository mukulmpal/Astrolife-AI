...
alter table public.profiles add column if not exists tob time;
...
drop policy if exists "usage_select_own" on public.usage_limits;
drop policy if exists "usage_insert_own" on public.usage_limits;
drop policy if exists "usage_update_own" on public.usage_limits;
