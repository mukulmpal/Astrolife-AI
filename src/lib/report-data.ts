...
export function buildPremiumReportData(input: any): PremiumReportData {
  ...
  return {
    ...
    moonSign,
    nakshatra,
    currentDasha: safeText(rawChart?.currentDasha ?? rawChart?.dasha?.current, "To be calculated"),
    reportTitle: "AstroLife Premium Kundli Report",
    executiveSummary: [
      `This premium report combines Vedic astrology, planetary positions, yogas, doshas, dasha timing, transits and AI-assisted interpretation to provide a deep view of your life pattern. The goal is not to create fear, but to give clarity about strengths, challenges, timing and practical remedies.`,
      `Your chart shows that life improves when emotional awareness is combined with disciplined execution. Some areas may demand patience, especially when karmic planets influence important houses, but the same pressure can become a source of maturity and long-term achievement.`,
    ...
    keyHighlights: [
      ...
      `Moon Sign: ${moonSign}`,
      `Nakshatra: ${nakshatra}`,
      `Current Dasha: ${safeText(rawChart?.currentDasha ?? rawChart?.dasha?.current, "To be calculated")}`,
