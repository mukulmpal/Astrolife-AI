...
// ── Nakshatra disease classical rules ────────────────────────────────────
...
export const PLANET_HOUSE_DISEASE: Record<string, Record<number, string>> = {
  Sun: {
    ...
    12:'Eye disease (12th Sun rule. Sleep disorder, vitality loss.',
  },
  Moon: {
    ...
    12:'Left eye weakness. Sleep disorder, mental fatigue, isolation.',
  },
  Mars: {
    ...
    7:'Emotional disease/BP. Relationship aggression. Sexual inflammation.',
  ...
  Mercury: {
    ...
    2:'Dental problems. Speech-related head/dental disease.',
    ...
    6:'TB risk. Mercury+Mars H6 → tuberculosis. Digestive nerve.',
    ...
    12:'Left ear weakness. Sleep-related nerve disorder. Foreign illness.',
  },
  Jupiter: {
    1:'Fat body tendency. Jupiter lagna + watery sign → fat. Liver size.',
    ...
    8:'Nephritis. Saturn+Sun+Venus H5 or Mars H10+Saturn. Transformation health.',
  ...
  Venus: {
    ...
    2:'Right eye. Venus+malefic H2/H12 → poor vision. Facial.',
    ...
    6:'Right eye affliction. Venus H6 → right eye problem.',
    7:'Venereal/reproductive disease. Kidney-bladder sensitivity.',
    ...
    12:'Night blindness. Venus+Moon H6-8-12 → night blindness. Hidden reproductive.',
  },
  Saturn: {
    ...
    4:'Lung weakness. Saturn H4 → depression. Chronic chest.',
    5:'Slow digestion, infertility, chronic stomach. Long disease.',
    6:'Arthritis, chronic disease. Saturn+Mars H6 → TB risk.',
  ...
  Rahu: {
    ...
    5:'Eye strain. Rahu H5+Sun aspect → eye destruction. Screen addiction.',
    6:'Mysterious undiagnosed disease. Rahu H6 → TB at 26.',
    7:'Sexual disease, obsessive relationship health. Rahu+Venus → venereal.',
  ...
  Ketu: {
    ...
    5:'Hidden eye issues, neural confusion. Rahu/Ketu H5 → vision.',
...
export const DISEASE_COMBOS_BOOK = [
  { key:'eye_saturn_2', disease:'Eye disease', note:'Saturn H2 → right eye affected', check:(p:Record<string,{house:number}>) => !!p.Saturn && p.Saturn.house===2 },
  { key:'eye_venus_6_8', disease:'Right eye problem', note:'Venus H6/H8 → right eye trouble', check:(p:Record<string,{house:number}>) => !!p.Venus && [6,8].includes(p.Venus.house) },
  { key:'eye_rahu_5', disease:'Eye destruction risk', note:'Rahu H5 → eye damage', check:(p:Record<string,{house:number}>) => !!p.Rahu && p.Rahu.house===5 },
  { key:'eye_sun_12', disease:'Eye lustre loss', note:'Sun H12 → eyes without brightness', check:(p:Record<string,{house:number}>) => !!p.Sun && p.Sun.house===12 },
  { key:'bp_mars_7', disease:'Blood Pressure / Emotional disease', note:'Mars H7 → BP and emotional disturbance', check:(p:Record<string,{house:number}>) => !!p.Mars && p.Mars.house===7 },
  { key:'mental_sat_1', disease:'Mental instability risk', note:'Saturn H1+Sun H12 → instability', check:(p:Record<string,{house:number}>) => !!p.Saturn && p.Saturn.house===1 && !!p.Sun && p.Sun.house===12 },
  { key:'anxiety_sat_mars', disease:'Anxiety / wavering mind', note:'Saturn H1+Mars H5/H9 → pavana prakopa yoga', check:(p:Record<string,{house:number}>) => !!p.Saturn && p.Saturn.house===1 && !!p.Mars && [5,9].includes(p.Mars.house) },
  ...
  { key:'tb_rahu_6', disease:'TB risk (~26 years)', note:'Rahu H6 → tuberculosis', check:(p:Record<string,{house:number}>) => !!p.Rahu && p.Rahu.house===6 },
  { key:'tb_mercury_mars_6', disease:'Tuberculosis risk', note:'Mercury+Mars H6 → TB', check:(p:Record<string,{house:number}>) => !!p.Mercury && !!p.Mars && p.Mercury.house===6 && p.Mars.house===6 },
  { key:'tb_saturn_mars_6', disease:'Tuberculosis risk', note:'Saturn+Mars H6 → TB', check:(p:Record<string,{house:number}>) => !!p.Saturn && !!p.Mars && p.Saturn.house===6 && p.Mars.house===6 },
  ...
  { key:'leucoderma', disease:'Leucoderma risk', note:'Saturn+Moon+Mars conjunct', check:(p:Record<string,{house:number}>) => !!p.Saturn && !!p.Moon && !!p.Mars && p.Saturn.house===p.Moon.house && p.Moon.house===p.Mars.house },
  { key:'sexual_venus_rahu', disease:'Sexual / reproductive disease', note:'Venus+Rahu conjunction → sexual disease', check:(p:Record<string,{house:number}>) => !!p.Venus && !!p.Rahu && p.Venus.house===p.Rahu.house },
  { key:'heart_moon_dusthana', disease:'Heart disease risk', note:'Moon in dusthana + 4th lord weak → heart', check:(p:Record<string,{house:number}>) => !!p.Moon && [6,8,12].includes(p.Moon.house) },
  { key:'nephritis_sun_mars', disease:'Nephritis', note:'Sun H1+Mars H6 → nephritis', check:(p:Record<string,{house:number}>) => !!p.Sun && p.Sun.house===1 && !!p.Mars && p.Mars.house===6 },
];

// ── Sign diseases — classical rules (Section 20) ─────────────────────
...
// ── Planet body zones for boils — classical rules (Section 19J) ───────
