...
function normalizeKundli(chart: unknown, source: string): NormalizedKundli {
  // Try flat string fields first, then extract from dashas array
  ...
  if (!result) {
    return (
      <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
        <h2 className="text-2xl font-bold">Complete Vastu Analysis</h2>
        <p className="mt-2 text-sm text-white/60">
          Generate or load your Kundli first. This analysis reads your chart planets and maps them to 16 Vastu zones.
        </p>
      </section>
    );
  }
    ...
    <>
      <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.25em] text-white/35">Legacy Kundli-Based Engine</p>
            <h2 className="mt-2 text-2xl font-bold">Complete Vastu Analysis</h2>
            <p className="mt-2 text-sm text-white/60">
              Chart planets mapped into 16 MahaVastu zones, with strengths, weak zones, remedies, psychology bridge and transit alerts.
            </p>
          </div>
          <div className="rounded-2xl border border-amber-400/25 bg-amber-400/10 p-4 text-center">
            <p className="text-xs uppercase tracking-widest text-amber-200/70">Overall</p>
            <p className={`mt-1 text-4xl font-bold ${scoreColor(result.overallScore)}`}>{result.overallScore}</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
          <p className="text-xs uppercase tracking-widest text-white/45">Total Zones</p>
          <p className="mt-2 text-3xl font-bold text-white">{result.zones.length}</p>
        </div>
        <div className="rounded-2xl border border-green-400/20 bg-green-500/5 p-4">
          <p className="text-xs uppercase tracking-widest text-green-200/60">Strong Zones</p>
          <p className="mt-2 text-3xl font-bold text-green-300">{result.strongZones.length}</p>
        </div>
        <div className="rounded-2xl border border-red-400/20 bg-red-500/5 p-4">
          <p className="text-xs uppercase tracking-widest text-red-200/60">Weak Zones</p>
          <p className="mt-2 text-3xl font-bold text-red-300">{result.weakZones.length}</p>
        </div>
        <div className="rounded-2xl border border-indigo-400/20 bg-indigo-500/5 p-4">
          <p className="text-xs uppercase tracking-widest text-indigo-200/60">Transit Alerts</p>
          <p className="mt-2 text-3xl font-bold text-indigo-300">{result.transitAlerts.length}</p>
        </div>
      </section>
      ...
      <section className="rounded-3xl border border-indigo-400/20 bg-indigo-500/5 p-6">
        <div className="flex flex-wrap gap-2">
          {(["all", "strong", "weak", "psych", "transit", "rooms"] as const).map((tab) => (
            ...
            >
              {tab === "psych" ? "Psych Bridge" : tab === "transit" ? "Transit Alerts" : tab === "rooms" ? "Room Guide" : tab}
            </button>
          ))}
        </div>
            ...
            {zones.map((zone) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-sm font-bold" style={{ color: zone.statusColor }}>{zone.score}</span>
                </div>
                <p className="mt-1 text-sm font-semibold text-white">{zone.name}</p>
                <p className="mt-0.5 text-xs text-white/45">{zone.planet} · {zone.element}</p>
                <p className="mt-2 text-xs text-white/40">{zone.domain}</p>
                {zone.planets.length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {zone.planets.map((planet) => (
                      <span key={planet} className="rounded-full bg-indigo-500/20 px-2 py-0.5 text-xs text-indigo-300">
                        {planet}
                      </span>
                    ))}
                  </div>
                )}
                <p className="mt-3 text-xs text-white/55">Ideal: {zone.roomIdeal}</p>
                <p className="mt-2 text-xs text-amber-200/80">Remedy: {zone.remedy}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {zoneTab === "psych" && (
          <div className="mt-5 space-y-3">
            {result.psychBridge.map((item, index) => (
              <div key={index} className="rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-white/70">
                {item}
              </div>
            ))}
          </div>
        ...
        {zoneTab === "transit" && (
          <div className="mt-5 space-y-3">
            {result.transitAlerts.length === 0 && <p className="text-sm text-white/50">No active transit alerts.</p>}
            {result.transitAlerts.map((alert, index) => (
              <div
                key={index}
                className={`rounded-xl border p-4 text-sm ${
                  alert.positive ? "border-green-400/25 bg-green-500/5" : "border-red-400/25 bg-red-500/5"
                }`}
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className={`font-bold ${alert.positive ? "text-green-300" : "text-red-300"}`}>{alert.planet}</span>
                  <span className="text-white/40">→</span>
                  <span className="text-white/70">{alert.zone}</span>
                  <span className="text-white/35">· {alert.domain}</span>
                </div>
                <p className="mt-2 text-white/60">{alert.effect}</p>
                <p className="mt-1 text-xs text-amber-200/75">Remedy: {alert.remedy}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {zoneTab === "rooms" && (
          <div className="mt-5 grid gap-3 md:grid-cols-2">
            {result.roomGuide.map((room) => (
              <div key={room.room} className="rounded-xl border border-white/10 bg-black/20 p-4">
                <p className="font-semibold text-white">{room.room}</p>
                <p className="mt-1 text-sm text-amber-200">{room.idealDir}</p>
                <p className="mt-2 text-sm text-white/60">{room.reason}</p>
              </div>
            ))}
          </div>
        )}
...
export default function VastuDashboardPage() {
  ...
  const [analysisMode, setAnalysisMode] = useState<"complete" | "calculator">("complete");
  ...
  useEffect(() => {
    void Promise.resolve().then(refreshChart);
  }, []);
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const data = (await response.json()) as VastuCalculatorResult;
      setResult(data);
    } catch (error) {
  ...
  return (
    <div className="page">
      <div style={{ maxWidth: "1200px", margin: "0 auto", width: "100%" }} className="flex flex-col gap-6">

        {/* Header */}
        <section>
          <div className="page-tag">{t("vastu.page_tag")}</div>
          <h1 className="page-title serif">{t("vastu.page_title")}</h1>
          <p className="page-sub">
            Property-based room directions · 16-zone MahaVastu · Kundli-safe remedies
          </p>
        </section>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          <div className="flex items-center justify-between">
            <p className="text-sm text-white/60">{chartLoadMessage}</p>
              <button
        ...
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-3">
          <div className="flex flex-wrap gap-2">
            {[
              { key: "complete", label: "Complete Analysis", desc: "Kundli based 16-zone Astro-Vastu" },
              { key: "calculator", label: "Vastu Calculator", desc: "Property, room and floor-plan inputs" },
            ].map((mode) => (
              <button
                key={mode.key}
                onClick={() => setAnalysisMode(mode.key as "complete" | "calculator")}
                className={`rounded-2xl px-5 py-3 text-left transition ${
                  analysisMode === mode.key
                    ? "bg-amber-400 text-black"
                    : "border border-white/10 bg-black/20 text-white/65 hover:bg-white/10"
                }`}
              >
        ...
        {analysisMode === "complete" ? (
          <CompleteAnalysisPanel result={completeAnalysis} />
        ) : (
          <>
        <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6">
